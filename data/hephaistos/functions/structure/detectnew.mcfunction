...
execute as @e[type=marker,tag=hs_structureverified] at @s positioned ~ ~ ~ if entity @e[distance=..8,limit=1,sort=nearest,type=marker,nbt={data:{ha:{item_catalyst:1b}}}] run kill @s
...
# 0 = not yet detected
# 1 = detected
# 2 = marker summoned / no structure / other structure too close
#
